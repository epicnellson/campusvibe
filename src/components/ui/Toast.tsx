...
function ToastItem({ toast, onDone }: { toast: ToastMessage; onDone: () => void }) {
  ...
  return (
    ...
    >
      <View style={[styles.iconWrap, { backgroundColor: colors.surfaceElement }]}>
        <Text style={[styles.icon, { color: colors.text }]}>{tc.icon}</Text>
      </View>
      <Text style={[styles.message, { color: colors.text }]} numberOfLines={2}>
        {toast.message}
      </Text>
      <TouchableOpacity onPress={onDone} style={styles.dismiss}>
        <Text style={[styles.dismissText, { color: colors.mutedLight }]}>✕</Text>
      </TouchableOpacity>
    </Animated.View>
  );
}
